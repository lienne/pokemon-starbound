...
#define temp_dexNum     vars[2]
    ...
    case ORDER_ALPHABETICAL:
        for (i = 0; i < ARRAY_COUNT(gPokedexOrder_Alphabetical); i++)
        {
            ...
            if (temp_dexNum <= NATIONAL_DEX_COUNT && (!temp_isHoennDex || NationalToAstreaOrder(temp_dexNum) != 0) && GetSetPokedexFlag(temp_dexNum, FLAG_GET_SEEN))
            {
    ...
    case ORDER_HEAVIEST:
        for (i = ARRAY_COUNT(gPokedexOrder_Weight) - 1; i >= 0; i--)
        {
            ...
            if (temp_dexNum <= NATIONAL_DEX_COUNT && (!temp_isHoennDex || NationalToAstreaOrder(temp_dexNum) != 0) && GetSetPokedexFlag(temp_dexNum, FLAG_GET_CAUGHT))
            {
                sPokedexView->pokedexList[sPokedexView->pokemonListCount].dexNum = temp_dexNum;
                sPokedexView->pokedexList[sPokedexView->pokemonListCount].seen = TRUE;
...
u16 GetAstreaPokedexCount(u8 caseID)
{
    u16 count = 0;
    u16 i;

    for (i = 0; i < ASTREA_DEX_COUNT - 1; i++)
    {
        switch (caseID)
        {
        case FLAG_GET_SEEN:
            if (GetSetPokedexFlag(AstreaToNationalOrder(i + 1), FLAG_GET_SEEN))
                count++;
            break;
        case FLAG_GET_CAUGHT:
            if (GetSetPokedexFlag(AstreaToNationalOrder(i + 1), FLAG_GET_CAUGHT))
                count++;
            break;
        }
    }
